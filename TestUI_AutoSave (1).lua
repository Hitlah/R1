...
local configFolder = "workspace/ALSHalloweenEvent"
...
if isfile(configFile) then
    ...
    if success and type(data) == "table" then
        ...
        warn("[AutoConfig] Loaded saved configuration.")
    else
        warn("[AutoConfig] Failed to decode config, resetting.")
    end
end
...
local function saveConfig()
    local success, data = pcall(function()
        writefile(configFile, HttpService:JSONEncode(getgenv()))
    end)
    if not success then
        warn("[AutoConfig] Failed to save configuration!")
    end
end

-- Wrap WindUI callbacks to auto-save
local oldCallback = WindUI and WindUI.Notify
local function autoSaveWrapper(callback)
    return function(...)
